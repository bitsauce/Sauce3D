...
{
public:
	StackWalkerInternal(Callstack *parent, HANDLE hProcess)
	{
		m_parent = parent;
		m_hDbhHelp = NULL;
		pSC = NULL;
	...
	BOOL Init(LPCSTR szSymPath)
	{
		...
		char buf[Callstack::STACKWALK_MAX_NAMELEN] = { 0 };
		if (this->pSGSP != NULL)
		{
			if (this->pSGSP(m_hProcess, buf, Callstack::STACKWALK_MAX_NAMELEN) == FALSE)
...
#pragma pack( pop )
	...
	DWORD LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
	{
		...
		if ((m_parent != NULL) && (szImg != NULL))
		{
			...
			if ((this->m_parent->m_options & Callstack::RetrieveFileVersion) != 0)
			{
...
// #############################################################
Callstack::Callstack(int options, LPCSTR szSymPath, DWORD dwProcessId, HANDLE hProcess)
{
...
static Callstack::PReadProcessMemoryRoutine s_readMemoryFunction = NULL;
static LPVOID s_readMemoryFunction_UserData = NULL;

BOOL Callstack::StoreCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
	CONTEXT c;
...
#error "Platform not supported!"
	...
	for (frameNum = 0; ; ++frameNum)
	{
		...
		if (frameNum > 0)
		{
			this->OnCallstackEntry(nextEntry, csEntry);
		}
...
BOOL __stdcall Callstack::myReadProcMem(
	HANDLE      hProcess,
...
void Callstack::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName, ULONGLONG fileVersion)
{
...
void Callstack::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
	...
	if ((eType != lastEntry) && (entry.offset != 0))
	{
		...
		OnOutput(buffer);
		m_callstackString += buffer;
	}
}

void Callstack::OnDbgHelpErr(LPCSTR szFuncName, DWORD gle, DWORD64 addr)
{
	CHAR buffer[STACKWALK_MAX_NAMELEN];
...
void Callstack::OnSymInit(LPCSTR szSearchPath, DWORD symOptions, LPCSTR szUserName)
{
	CHAR buffer[STACKWALK_MAX_NAMELEN];
...
void Callstack::OnOutput(LPCSTR buffer)
{
